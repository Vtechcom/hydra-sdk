...
## 大量トランザクションの構築（v1.2.0 以降）

すべての CSL オブジェクトは WASM メモリに存在し、`.free()` によってのみ回収されます。単発のトランザクションであればデフォルトの `complete()` で問題ありませんが、数千件を連続して構築する場合（例: Hydra Head 内）は、`@hydra-sdk/transaction@1.2.0` で追加されたリークのないヘルパーを使用してください:

- **`completeCbor()`** — CBOR hex を構築して返し、中間の `Transaction` を代わりに解放します。バイトのみが必要な場合は、`complete()` + `.to_hex()` よりもこちらを優先してください。
- **`dispose()` / `using`** — ビルダーが保持するすべての WASM メモリを解放します。1 つのビルダーをスパイクの間 `reset()` で再利用し、最後に `dispose()` します。
...
ベンチマークと完全なパターン一覧については、[Performance → TxBuilder WASM Memory](/resources/performance#txbuilder-wasm-memory-v120) を参照してください。

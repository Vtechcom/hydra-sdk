...
Đóng Head chỉ khi trong đó không còn asset nào ngoài ADA. Node trả về `InvalidInput` nếu vẫn còn asset.
...
Side-load một snapshot đã confirm vào Head.
...
Fanout một tập UTxO do client chọn từ Head đã đóng.

::warning
Cần hydra-node mới hơn 2.3.0. Node cũ trả về `InvalidInput`.
::
...
Gửi transaction L2 qua `POST /transaction` và để node trả về kết quả, thay vì đua với WebSocket message theo timeout phía client. Nên dùng thay cho `submitTxSync()`.
...
Transaction id của các deposit đã thấy on-chain nhưng chưa vào snapshot.
...
Recover một deposit đang chờ về lại L1. Deposit vẫn recover được sau khi Head đóng, nên đây cũng là cách lấy lại tiền từ Head trước đó.
...
Protocol parameters nguyên bản từ node — gồm cả `costModels` và `protocolVersion` mà `getProtocolParameters()` bỏ đi. Dùng khi cần tính ExUnits cho Plutus.

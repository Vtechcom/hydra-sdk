...
description: Các ví dụ tích hợp với Hydra Network bằng những mẫu thực tế
navigation:
  icon: i-lucide-network
---

Các ví dụ dựa trên những mẫu tích hợp thực tế được dùng trong hệ sinh thái Hydra SDK.

## Xây dựng giao dịch Hydra cơ bản
...
## Kết nối Hydra Bridge (từ các mẫu sử dụng thực tế)
...
## Tích hợp API tùy chỉnh với Hydra
...
## Ví dụ tích hợp Hydra hoàn chỉnh
...
Gửi kiểu fire-and-forget không chặn (non-blocking) dùng mẫu callback error-first của Node.js:
...
### Khi nào dùng `submitTx` so với `submitTxSync`

| | `submitTxSync` | `submitTx` |
|---|---|---|
| Trả về | `Promise<TxResult>` | `void` |
| Cách dùng | `await bridge.submitTxSync(tx)` | dựa trên callback |
| Phù hợp nhất cho | Luồng tuần tự, `async/await` | Vòng lặp sự kiện, fire-and-forget |

---

## Bộ nhớ đệm số dư trong bộ nhớ (v1.3.0+)

Sau `Greetings` hoặc `SnapshotConfirmed` đầu tiên, việc đọc số dư có độ phức tạp O(1):
...
## Những lỗi Hydra thường gặp và cách giải quyết

### 1. Sai chế độ giao dịch
```typescript

...
## Xây dựng khối lượng lớn (v1.2.0+)

Mọi đối tượng CSL đều nằm trong bộ nhớ WASM và chỉ được thu hồi bằng `.free()`. Đối với các giao dịch đơn lẻ thì `complete()` mặc định là ổn, nhưng khi bạn xây dựng hàng nghìn giao dịch liên tiếp (ví dụ bên trong một Hydra Head), hãy dùng các helper không rò rỉ được thêm trong `@hydra-sdk/transaction@1.2.0`:

- **`completeCbor()`** — xây dựng và trả về CBOR hex, giải phóng `Transaction` trung gian giúp bạn. Hãy ưu tiên nó thay vì `complete()` + `.to_hex()` khi bạn chỉ cần các byte.
- **`dispose()` / `using`** — giải phóng toàn bộ bộ nhớ WASM mà một builder đang giữ; tái sử dụng một builder với `reset()` qua một spike và `dispose()` ở cuối.
...
Xem [Hiệu năng → Bộ nhớ WASM của TxBuilder](/resources/performance#txbuilder-wasm-memory-v120) để biết benchmark và danh sách mẫu đầy đủ.

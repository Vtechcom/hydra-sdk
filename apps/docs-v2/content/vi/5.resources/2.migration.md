---
title: Migration
description: Các đường nâng cấp cho Hydra SDK — bridge v1.x → v2.0, và hệ thống utilities v1.1.x trước đó.
navigation:
  icon: i-lucide-arrow-right-left
---

## `@hydra-sdk/bridge` v1.x → v2.0

`@hydra-sdk/bridge` **2.0** nhắm tới `hydra-node` **2.x**. Đây là breaking change, và bridge **1.x đã ngừng phát triển**.

::warning
v2 phát hành dưới dist-tag `next` — `latest` vẫn trỏ về 1.3.2.
::

```bash
pnpm add @hydra-sdk/bridge@next
```

### Vì sao breaking

hydra-node v2 hiện thực ADR-33 và **bỏ commit phase**. Head giờ mở thẳng, nên không còn gì để abort và không có bước `Committed`; tiền vào head đang mở qua **incremental deposit**.

### Bạn cần đổi gì

| v1.x | v2.0 |
| --- | --- |
| `bridge.commands.abort()` | Đã xoá — không còn commit phase để abort |
| `bridge.commands.initSync()` | Resolve khi nhận `HeadIsOpen`, không phải `HeadIsInitializing` |
| Handler `HeadIsOpen` | Đọc `parties`; `utxo` không còn — lấy UTxO từ `SnapshotConfirmed` hoặc `querySnapshotUtxo()` |
| `HeadIsFinalized.utxo` | Đổi tên thành `finalizedUTxO` |
| `HydraHeadStatus` | Không còn `Initializing` / `Final` |
| `HydraHeadInfo` | Giờ là union trên `Idle | Open | Closed` — narrow theo `tag` trước |
| `Greetings` / `ClientMessage` | Không có `seq` / `timestamp` — chúng được gửi untimed |

### Chi tiết đầy đủ

Danh sách đầy đủ, gồm các API mới (`safeClose()`, `submitL2Tx()`, `pendingDeposits()`, `recoverDeposit()`) và các fix wire-format tìm ra khi test với node thật, nằm ở [MIGRATION-v2.md](https://github.com/Vtechcom/hydra-sdk/blob/master/packages/hydra-bridge/MIGRATION-v2.md).

---

## Migration lên v1.1.x (cũ)

Hướng dẫn này giúp bạn nâng cấp từ các phiên bản Hydra SDK trước lên v1.1.x, phiên bản giới thiệu một hệ thống utilities mới và trải nghiệm nhà phát triển được cải thiện.

## Có gì thay đổi

### Hệ thống utilities mới

Phiên bản 1.1.0 giới thiệu một hệ thống utilities được cải tổ hoàn toàn với các namespace được tổ chức:

**Trước (v1.0.x):**
```typescript
// Individual utility imports (if available)
import { someUtility } from '@hydra-sdk/core/utils/some-utility'
```

**Sau (v1.1.x):**
```typescript
// Organized namespace imports
import { 
  ParserUtils,
  TimeUtils,
  DatumUtils,
  PolicyUtils,
  Serializer,
  Deserializer 
} from '@hydra-sdk/core'
```

## Thay đổi phá vỡ tương thích

### 1. Tổ chức hàm utility

**Trước:**
```typescript
// Direct imports from utils (if they existed)
import { hexToBytes, bytesToHex } from '@hydra-sdk/core/utils'
```

**Sau:**
```typescript
// Namespaced utility imports
import { ParserUtils } from '@hydra-sdk/core'

const hex = ParserUtils.bytesToHex(bytes)
const bytes = ParserUtils.hexToBytes(hex)
```

### 2. Utilities Time/Slot

**Trước:**
```typescript
// Manual slot calculations or external libraries
const currentSlot = calculateSlot(Date.now())
```

**Sau:**
```typescript
import { TimeUtils, SLOT_CONFIG_NETWORK } from '@hydra-sdk/core'
const currentSlot = TimeUtils.unixTimeToEnclosingSlot(
    Date.now(), 
    SLOT_CONFIG_NETWORK.PREPROD
)
const futureSlot = TimeUtils.unixTimeToEnclosingSlot(
    Date.now() + 3600000, 
    SLOT_CONFIG_NETWORK.PREPROD
)
```

### 3. Tạo Datum

**Trước:**
```typescript
// Manual WASM operations
import { CardanoWASM } from '@hydra-sdk/cardano-wasm'

const datum = CardanoWASM.PlutusData.new_integer(
  CardanoWASM.BigInt.from_str('42')
)
```

**Sau:**
```typescript
import { DatumUtils } from '@hydra-sdk/core'

const datum = DatumUtils.mkInt(42)
const bytesDatum = DatumUtils.mkBytes('deadbeef')
const constrDatum = DatumUtils.mkConstr(0, [datum, bytesDatum])
```

## Các bước migration

### Bước 1: Cập nhật Dependencies

```bash
# Update to latest version
npm update @hydra-sdk/core @hydra-sdk/bridge @hydra-sdk/transaction @hydra-sdk/cardano-wasm

# Or with specific version
npm install @hydra-sdk/core@^1.1.0 @hydra-sdk/cardano-wasm
```

### Bước 2: Cập nhật Imports

Thay thế các import utility riêng lẻ bằng namespace import:

```typescript
// OLD - Remove these
import { someUtility } from '@hydra-sdk/core/utils/some-utility'
import { CardanoWASM } from '@hydra-sdk/cardano-wasm'

// NEW - Use these instead
import { 
  ParserUtils,
  TimeUtils,
  DatumUtils,
  PolicyUtils,
  Serializer,
  Deserializer,
  Resolver,
  Converter,
  BuildKeys,
  ProviderUtils,
  CostModels 
} from '@hydra-sdk/core'
```

### Bước 3: Thay thế các thao tác thủ công

#### Chuyển đổi dữ liệu
```typescript
// OLD
const hex = Buffer.from(bytes).toString('hex')
const bytes = Buffer.from(hex, 'hex')

// NEW
import { ParserUtils } from '@hydra-sdk/core'
const hex = ParserUtils.bytesToHex(bytes)
const bytes = ParserUtils.hexToBytes(hex)
```

#### Tính toán thời gian
```typescript
// OLD
const slot = Math.floor((Date.now() - SHELLEY_START) / SLOT_LENGTH) + SHELLEY_SLOT_START

// NEW
import { TimeUtils, SLOT_CONFIG_NETWORK } from '@hydra-sdk/core'
const slot = TimeUtils.unixTimeToEnclosingSlot(Date.now(), SLOT_CONFIG_NETWORK.PREPROD)
```

#### Tạo Datum
```typescript
// OLD - Complex WASM operations
import { CardanoWASM } from '@hydra-sdk/cardano-wasm'
const datum = CardanoWASM.PlutusData.new_constr_plutus_data(
  CardanoWASM.ConstrPlutusData.new(
    CardanoWASM.BigNum.from_str('0'),
    list
  )
)

// NEW - Simple utility functions
import { DatumUtils } from '@hydra-sdk/core'
const datum = DatumUtils.mkConstr(0, [field1, field2])
```

### Bước 4: Cập nhật cách dùng Provider

Nếu bạn đang dùng các triển khai provider tùy chỉnh:

```typescript
// OLD - Custom provider setup
class MyProvider {
  async getUtxos(address) {
    // Custom implementation
  }
}

// NEW - Use built-in providers or extend base
import { ProviderUtils } from '@hydra-sdk/core'

const provider = new ProviderUtils.BlockfrostProvider({
  apiKey: 'your-blockfrost-api-key',
  network: 'preprod'
})

// Or extend a built-in provider to customize behavior
class MyProvider extends ProviderUtils.BlockfrostProvider {
  constructor() {
    super({ apiKey: 'your-blockfrost-api-key', network: 'preprod' })
  }

  // Inherited `.fetcher` / `.submitter` still work
  async getUtxos(address: string) {
    return this.fetcher.fetchAddressUTxOs(address)
  }
}
```

## Các mẫu migration phổ biến

### Mẫu 1: Tạo Metadata

**Trước:**
```typescript
const metadata = {
  721: {
    [policyId]: {
      [tokenName]: {
        name: Buffer.from(name).toString('hex'),
        image: Buffer.from(image).toString('hex')
      }
    }
  }
}
```

**Sau:**
```typescript
import { ParserUtils, DatumUtils } from '@hydra-sdk/core'

const metadata = {
  721: {
    [policyId]: {
      [tokenName]: {
        name: ParserUtils.stringToHex(name),
        image: ParserUtils.stringToHex(image)
      }
    }
  }
}

// Or create as datum
const metadataDatum = DatumUtils.mkMap([
  [DatumUtils.mkBytes(ParserUtils.stringToHex("name")), 
   DatumUtils.mkBytes(ParserUtils.stringToHex(name))],
  [DatumUtils.mkBytes(ParserUtils.stringToHex("image")), 
   DatumUtils.mkBytes(ParserUtils.stringToHex(image))]
])
```

### Mẫu 2: Thời điểm giao dịch

**Trước:**
```typescript
// Manual slot calculation
const validFrom = getCurrentSlot()
const validUntil = validFrom + 7200 // 1 hour in slots
```

**Sau:**
```typescript
import { TimeUtils, SLOT_CONFIG_NETWORK } from '@hydra-sdk/core'

const validFrom = TimeUtils.unixTimeToEnclosingSlot(Date.now(), SLOT_CONFIG_NETWORK.PREPROD)
const validUntil = TimeUtils.unixTimeToEnclosingSlot(Date.now() + 3600000, SLOT_CONFIG_NETWORK.PREPROD)
```

### Mẫu 3: Tạo Policy Script

**Trước:**
```typescript
// Manual WASM operations for policy
import { CardanoWASM } from '@hydra-sdk/cardano-wasm'

const keyHash = CardanoWASM.Ed25519KeyHash.from_hex(publicKeyHash)
const scriptPubkey = CardanoWASM.ScriptPubkey.new(keyHash)
const policy = CardanoWASM.NativeScript.new_script_pubkey(scriptPubkey)
```

**Sau:**
```typescript
import { PolicyUtils } from '@hydra-sdk/core'

const policy = PolicyUtils.buildPolicyScriptFromPubkey({
  type: 'sig',
  keyHash: publicKeyHash
})

// Or from address
const policy = PolicyUtils.buildMintingPolicyScriptFromAddress(address)
```

## Kiểm thử quá trình migration của bạn

### 1. Kiểm thử chức năng cơ bản

```typescript
import { 
  AppWallet, 
  NETWORK_ID,
  ParserUtils,
  TimeUtils,
  DatumUtils 
} from '@hydra-sdk/core'

// Test wallet creation (should work as before)
const wallet = new AppWallet({
  networkId: NETWORK_ID.PREPROD,
  key: { type: 'mnemonic', words: AppWallet.brew() }
})

// Test new utilities
const hex = ParserUtils.stringToHex('test')
const slot = TimeUtils.resolveSlotNo('preprod')
const datum = DatumUtils.mkInt(42)

console.log('Migration test passed:', {
  address: wallet.getAccount(0, 0).baseAddressBech32,
  hex,
  slot,
  datumHex: datum.to_hex()
})
```

### 2. Kiểm thử chuyển đổi dữ liệu

```typescript
import { ParserUtils } from '@hydra-sdk/core'

const testData = [
  'Hello World',
  '🚀 Hydra SDK',
  'Special chars: åøæ'
]

testData.forEach(str => {
  const hex = ParserUtils.stringToHex(str)
  const back = Buffer.from(hex, 'hex').toString('utf8')
  
  console.assert(str === back, `Conversion failed for: ${str}`)
})

console.log('Data conversion tests passed')
```

### 3. Kiểm thử Time Utilities

```typescript
import { TimeUtils, SLOT_CONFIG_NETWORK } from '@hydra-sdk/core'

const networks = ['MAINNET', 'PREPROD', 'PREVIEW'] as const

networks.forEach(network => {
  const slot = TimeUtils.unixTimeToEnclosingSlot(Date.now(), SLOT_CONFIG_NETWORK[network])
  console.assert(typeof slot === 'number' && slot > 0, `Invalid slot for network: ${network}`)
})

console.log('Time utilities tests passed')
```

## Xử lý sự cố

### Các vấn đề thường gặp

1. **Lỗi Import**
   ```
   Error: Module not found: @hydra-sdk/core/utils/...
   ```
   **Giải pháp**: Dùng namespace import thay vì deep import.

2. **Lỗi kiểu với Utilities**
   ```
   Error: Property 'stringToHex' does not exist on type...
   ```
   **Giải pháp**: Import đúng namespace: `ParserUtils.stringToHex`

3. **Vấn đề tính toán Slot**
   ```
   Error: Invalid network parameter
   ```
   **Giải pháp**: Dùng đúng chuỗi network: `'mainnet'`, `'preprod'`, hoặc `'preview'`

### Nhận trợ giúp

Nếu bạn gặp vấn đề trong quá trình migration:

1. Xem [Tham chiếu API Utilities](/api/utilities)
2. Xem lại [Hướng dẫn Working with Utilities](/guides/utilities-cookbook)
3. Tham khảo [Ví dụ Utilities](/guides/utilities-cookbook)
4. Tham gia Discord cộng đồng của chúng tôi để được hỗ trợ

## Lợi ích của v1.1.0

Sau khi migration, bạn sẽ có:

- **Trải nghiệm nhà phát triển tốt hơn**: Các hàm utility được tổ chức, dễ đoán
- **Type Safety**: Định nghĩa TypeScript toàn diện
- **Hiệu năng**: Các triển khai utility được tối ưu
- **Khả năng bảo trì**: Code sạch hơn, dễ đọc hơn
- **Sẵn sàng cho tương lai**: Nền tảng cho các tính năng sắp tới

Công sức migration được đền đáp bằng trải nghiệm phát triển và chất lượng code được cải thiện đáng kể.

...
::warning
**`@hydra-sdk/bridge` v2 is on the `next` tag.** v2 targets `hydra-node` **2.x** and is a breaking change; the `latest` tag still resolves to **1.3.2**, which speaks `hydra-node` **1.x**.

Running a v2 node? Install explicitly:

```bash
pnpm add @hydra-sdk/bridge@next
```

See the [migration guide](/resources/migration) before upgrading.
::

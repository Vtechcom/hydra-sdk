...
**Returns:** `CardanoCLiVkey | CardanoCliExtendedVkey`
Corresponding verification key. A BIP32-Ed25519 signing key (`5880...`) carries a chain code, so it yields an extended verification key (`5840...`); every other shape yields a plain one (`5820...`).

---

### **`KeysUtils.mnemonicToCliKey(mnemonic: string[], accountIndex?: number, keyIndex?: number)`**

**→ `{ sk: CardanoCliExtendedSkey; vk: CardanoCliExtendedVkey }`** :badge[1.1.6+]{variant="success"}

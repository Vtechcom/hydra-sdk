---
title: 'Ứng Dụng Đầy Đủ với Hydra SDK'
description: Ví dụ ứng dụng hoàn chỉnh tích hợp Hydra SDK với quản lý wallet, giao dịch, và Hydra Bridge
---

# Ứng Dụng Đầy Đủ với Hydra SDK

Ví dụ này minh họa cách xây dựng một ứng dụng hoàn chỉnh tích hợp tất cả các thành phần của Hydra SDK.
...
export class WalletService {
    ...
    getWallet(): AppWallet {
        if (!this.wallet || !this.isInitialized) {
            throw new Error('Wallet chưa được khởi tạo')
        }
        return this.wallet
    }
    ...
    async queryUTxOs(address: string): Promise<UTxO[]> {
        // Tích hợp với Blockfrost, Hexcore, hoặc dịch vụ khác
...
export class HydraService extends EventEmitter {
    ...
    async connect(): Promise<void> {
        try {
            ...
            console.error('Lỗi kết nối Hydra:', error)
    ...
    private setupEventListeners(): void {
        ...
        this.bridge.events.on('onError', (error) => {
            console.error('Lỗi Hydra Bridge:', error)
            this.emit('error', error)
        })
    ...
    async initHead(): Promise<void> {
        if (!this.bridge || !this.isConnected) {
            throw new Error('Chưa kết nối tới Hydra Bridge')
        }

        if (this.headStatus !== 'idle') {
            throw new Error(`Không thể init head ở trạng thái: ${this.headStatus}`)
        }
    ...
    async closeHead(): Promise<void> {
        ...
        if (this.headStatus !== 'open') {
            throw new Error(`Không thể close head ở trạng thái: ${this.headStatus}`)
        }
    ...
    async submitTransaction(txCbor: string, txId: string): Promise<any> {
        ...
        if (this.headStatus !== 'open') {
            throw new Error('Head chưa mở để xử lý giao dịch')
        }
...
export class TransactionService {
    async sendTransaction(request: TransactionRequest): Promise<string> {
        ...
        try {
            ...
            if (request.isHydra) {
                ...
                // Gửi lên L1 thông qua API
                await this.submitToL1(signedTx)
            }

            return txHash
        } catch (error) {
            console.error('Lỗi gửi transaction:', error)
            throw error
        }
    }
    ...
    private async submitToL1(txCbor: string): Promise<void> {
        ...
        if (!response.ok) {
            throw new Error('Gửi transaction thất bại')
        }
    }
    ...
    async buildContractTransaction(
        ...
        if (!collateralUTxO) {
            throw new Error('Không tìm thấy collateral UTxO phù hợp')
        }
    ...
    <div class="min-h-screen bg-gray-50">
        <header class="bg-white shadow-sm border-b">
            <div class="max-w-6xl mx-auto px-4 py-6">
                ...
                <p class="mt-2 text-gray-600">
                    Ứng dụng ví Cardano tích hợp Hydra Layer 2
                </p>
            </div>
        </header>
        ...
        <main class="max-w-6xl mx-auto px-4 py-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                ...
                <div class="bg-white rounded-lg shadow p-4">
                    <h3 class="text-sm font-medium text-gray-500">Số Dư</h3>
            ...
            <div class="mb-6">
                <nav class="flex space-x-8">
                    ...
                    >
                        {{ tab === 'wallet' ? 'Ví' : tab === 'hydra' ? 'Hydra' : 'Giao Dịch' }}
                    </button>
                </nav>
            </div>
...
```json
{
  ...
  "version": "1.0.0",
  "description": "Ứng dụng ví Cardano tích hợp Hydra SDK",
  "scripts": {
    "dev": "vite",
...
### 1. **Quản Lý Wallet Hoàn Chỉnh**
- Tạo/khôi phục wallet từ mnemonic
- Hiển thị địa chỉ và balance
- Quản lý UTxOs và assets

### 2. **Tích Hợp Hydra**
- Kết nối Hydra Bridge/Hexcore
- Quản lý vòng đời Head
...
1. **Service Layer Pattern**: Tách biệt business logic
2. **Event-Driven Architecture**: Loose coupling giữa components
3. **Error Boundary**: Comprehensive error handling
4. **State Management**: Centralized và reactive
5. **TypeScript**: Type safety toàn bộ codebase
6. **Modular Design**: Reusable components và services

Ứng dụng này minh họa cách tích hợp toàn bộ Hydra SDK vào một dự án thực tế với architecture tốt và user experience mượt mà.

---
title: Ví Dụ Ký Transaction
description: Các pattern ký transaction dựa trên implementation thực tế
---

# Ví Dụ Ký Transaction

Các pattern ký transaction cốt lõi dựa trên implementation thực tế từ `nodejs-playground/src`.
...
async function signTransaction(cborHex: string, partialSign: boolean = false) {
  try {
    ...
    console.log('✅ Transaction đã ký:', txHash)
...
class MultiSigWallet {
  ...
  async signWithAllWallets(cborHex: string) {
    ...
    for (let i = 0; i < this.wallets.length; i++) {
      ...
      console.log(`✅ Ví ${i + 1}/${this.wallets.length} đã ký`)
    }
  ...
  async signWithRetry(cborHex: string) {
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
      try {
        ...
        console.log(`✅ Ký thành công ở lần thử ${attempt}`)
    ...
    throw new Error(`Ký thất bại sau ${this.retryAttempts} lần thử: ${lastError.message}`)
  }
...
class PerformanceSigner {
  ...
  async signWithMetrics(cborHex: string) {
    ...
    try {
      ...
      console.log(`⏱️ Ký trong ${duration}ms: ${txHash}`)
      return { signedTx, txHash, duration }
      
    } catch (error) {
      const duration = Date.now() - startTime
      console.error(`❌ Ký thất bại sau ${duration}ms:`, error)
      throw error
    }
  }

...
description: Tìm hiểu cách commit UTxOs vào Hydra Head để kích hoạt transactions Layer 2
---

# Commit vào Hydra

Commit UTxOs vào Hydra Head là quá trình chuyển assets từ Cardano Layer 1 vào một state channel Hydra Head. Điều này cho phép transactions nhanh, chi phí thấp trong head.
...
export class HydraApi {
	...
	static async submitCardanoTx(tx: { cborHex: string; description: string; type: string }) {
		try {
			// Gửi CBOR đã ký lên mạng Cardano Layer 1 thông qua Hydra node
...
async function main() {
	...
	if (!hasAsset) {
		throw new Error(`Không tìm thấy asset ${depositAssetUnits.join(', ')} trong ví`)
	}

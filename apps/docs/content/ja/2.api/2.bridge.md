...
- `options` (InitHydraBridgeOptions):
  ...
  - `verbose?`: 詳細ログモードを有効化
  - `history?`: 完全なノード履歴の受信を有効化 (デフォルト: false)
  - `noSnapshotUtxo?`: スナップショットUTxOの可視性を無効化 (デフォルト: false)
  - `address?`: 特定のアドレスでトランザクション出力をフィルタリング
  - `autoReconnect?`: 切断時に自動再接続を有効化 (デフォルト: false)
  - `reconnectInterval?`: 再接続試行間の待機時間 (ミリ秒、デフォルト: 3000)
  - `maxReconnectAttempts?`: 最大再接続試行回数 (デフォルト: 0 = 無制限)
...
Hydra HeadのスロットゼロのUnixタイムスタンプ (ミリ秒)。`Greetings` イベント受信後に設定されます。スロット ↔ 時刻の変換に使用します。
...
最後に確認されたスナップショットの番号。`SnapshotConfirmed` イベントごとに更新されます。初期値は `0`。
...
bridge.events.on('onMessage', payload => {
	if (payload.tag === 'SnapshotConfirmed') {
		console.log('Latest snapshot:', bridge.lastSnapshotNumber)
	}
})
```
...
インメモリキャッシュからアドレスの残高をO(1)で取得します。最後のスナップショット時点の残高を返します。
...
if (balance) {
	...
	console.log('ADA balance:', lovelace)
} else {
	console.log('Address not found in current snapshot')
}
```

**戻り値:** `Map<string, bigint> | null`
アセットユニットから数量へのマップ。アドレスが見つからない場合は `null`
...
### **`HydraBridge.submitTx(transaction: Transaction, callback: (error: SubmitTxError | null, result: TxResult | null) => void, options?: SubmitOptions) → void`**

Node.js エラーファーストコールバックパターンを使用してトランザクションを送信します。Fire-and-forget スタイルの送信に適しています。

**パラメータ:**

- `transaction` (Transaction): 送信するトランザクション
- `callback` ((error: SubmitTxError | null, result: TxResult | null) => void): 結果コールバック。エラーファーストパターン
- `options?` (SubmitOptions): 送信オプション
  - `timeout?`: タイムアウト (ミリ秒)
...
- `history?` (boolean): 完全なノード履歴の受信を有効化 (デフォルト: false)
...
// コンストラクタは位置引数です: new HexcoreConnector(socketIoUrl, options?)
	...
	switch (payload.tag) {
		case 'Greetings':
			console.log('Greeting from Head:', {
				...
				snapshotNumber: payload.snapshotNumber,       // v1.3.0+
				hydraNodeVersion: payload.hydraNodeVersion,   // v1.3.0+
				timestamp: payload.timestamp,
				me: payload.me                                // このノードのvkey情報
			})
			break
...
```typescript
interface InitHydraBridgeOptions {
	url?: string
	...
	address?: string
	autoReconnect?: boolean        // 切断時に自動再接続 (デフォルト: false)
	reconnectInterval?: number     // 再接続間隔 ミリ秒 (デフォルト: 3000)
	maxReconnectAttempts?: number  // 最大試行回数 (デフォルト: 0 = 無制限)
}
```
...
`submitTx` コールバックで返される `TxResult` と同じ型です:
...
5. **リソースクリーンアップ**: 完了したら常に`disconnect()`を呼び出します (`autoReconnect` ループも安全に停止します)
6. **トランザクション検証**: 送信前にトランザクションを検証します
7. **タイムアウト設定**: 操作に適切なタイムアウトを設定します
8. **ログ記録**: 開発中のデバッグのため詳細モードを有効化します
9. **インメモリキャッシュ**: `getAddressBalance()` を使用してO(1)で残高を取得します。`queryAddressUTxO()` の代わりにホットパスで活用できます
10. **自動再接続**: 本番環境では `autoReconnect: true` と適切な `reconnectInterval` を設定します。`disconnect()` を呼び出すと再接続ループは安全に停止します
11. **submitTx vs submitTxSync**: ウォレット等でファイヤー・アンド・フォーゲットのフローには `submitTx` コールバックAPIを使用します。確認を待つ場合は `submitTxSync` を使用します

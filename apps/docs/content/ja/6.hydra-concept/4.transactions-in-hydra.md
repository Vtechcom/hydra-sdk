...
### 代替手段：submitTx コールバック（v1.3.0+）

`submitTxSync` の代わりに、Node.js エラーファーストコールバックスタイルの `submitTx` を使用することもできます。

```typescript
// src/build-tx-callback.ts
bridge.submitTx(
	{
		cborHex: signedCbor,
		type: 'Witnessed Tx ConwayEra',
	...
	(error, result) => {
		if (error) {
			console.error('>>> Submit tx error:', error.message)
			return
		}
...
`submitTx` は `void` を返します。確認を待たずに他の処理を続行できます。複数のトランザクションを並行して送信する場合に便利です。
